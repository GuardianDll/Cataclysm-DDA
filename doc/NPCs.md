...
| best_tool(`s`/`v`)    |  ✅   |   ❌  | u, n | Return the level of the best tool you have of specified type. Argument is tool type.  Optional kwargs:<br/>`radius`: `d/v` - how big the search radius around the character; Default is PICKUP_RANGE (6 tiles); 0 would check only items in character inventory.<br/><br/> Example:<br/>`"condition": { "math": [ "u_best_tool('BUTCHER', 'range': 2) > 10" ] }`|
