...
void pulp_activity_actor::start( player_activity &act, Character &you )
{
    ...
    if( pair_bash.second.has_flag( flag_MESSY ) ) {
        mess_radius = 2;
    }

    const double weight_factor = units::to_kilogram( you.get_weight() ) / 10;
    const double athletic_factor = std::min( 6.0f, you.get_skill_level( skill_swimming ) + 1 ) * 3;
    const double strength_factor = you.get_str() / 2;
    ...
    you.add_msg_if_player( m_info,
                           ...
                           bash_factor, bash_factor * bash_factor );

    bash_factor = std::pow( bash_factor, 2 );
    const std::pair<int, item> pair_cut;
    if( you.max_quality( qual_BUTCHER ) > 5 ) {
    ...
    pulp_power = bash_factor
                 * ( std::sqrt( you.get_skill_level( skill_survival ) + 1 ) )
                 * ( can_severe_cutting ? 1 : 0.85 );

    pulp_effort = get_option<float>( "PLAYER_BASE_STAMINA_REGEN_RATE" ) * 2.5;
...
void pulp_activity_actor::do_turn( player_activity &act, Character &you )
{
    ...
    for( item &corpse : corpse_pile ) {
        ...
        if( corpse.damage() < corpse.max_damage() ) {
            bool can_pulp = punch_corpse_once( corpse, act, you, pos, here );
            if( !can_pulp ) {
...
bool pulp_activity_actor::punch_corpse_once( item &corpse, player_activity &act, Character &you,
        tripoint_bub_ms pos, map &here )
{
    const mtype *corpse_mtype = corpse.get_mtype();

    // if something is so big, chances are it has hide and muscles strong enough
    // that you can't stomp it without a better tool
    // threshold is 30 bash for size 4, bash 45 for size 5
    if( corpse_mtype->size == 4 && bash_factor < std::pow( 33, 2 ) ) {
        too_big_corpse = true;
        return false;
    }
    if( corpse_mtype->size == 5 && bash_factor < std::pow( 45, 2 ) ) {
        too_big_corpse = true;
        return false;
    }

    int time_to_pulp = units::to_milliliter( corpse.volume() ) / pulp_power;
    ...
    // 2.5 times longer if acid, to not get sprayed by accident
    if( acid_corpse ) {
        time_to_pulp *= 2.5;
    }
    ...
    add_msg_debug( debugmode::DF_ACTIVITY,
                   "Activity: %s, corpse: %s, time to pulp: %s, bash tool: %s, cut tool: %s, pry tool: %s",
                   act.id().str(), corpse_mtype->id.str(), to_string( time_duration::from_seconds( time_to_pulp ) ),
                   bash_tool, cut_tool, pry_tool );

    // how much damage you need to deal each second to match expected pulp time
    const int corpse_damage = 4000 / time_to_pulp;
    ...
    if( one_in( 16 ) ) {
        ...
        you.practice( skill_swimming, 2, 2 );
        if( unknown_prof.has_value() ) {
            you.practice_proficiency( unknown_prof.value(), 2_seconds );
        }
    }
...
void pulp_activity_actor::send_final_message( Character &you ) const
{
    ...
    if( too_big_corpse && num_corpses == 0 ) {
        you.add_msg_player_or_npc( n_gettext(
                                       _( "You cannot pulp this corpse, for you have no tool heavy enough to deal with it." ),
                                       _( "You cannot pulp these corpses, for you have no tool heavy enough to deal with it." ),
                                       unpulped_corpses_qty ),
                                   n_gettext(
